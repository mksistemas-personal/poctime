...
@Transactional
@AllArgsConstructor
class DeleteEconomicGroupService implements DeleteBusinessUseCase<Tsid, EconomicGroupResponse> {
    ...
    public EconomicGroupResponse execute(Tsid id) {
        return (EconomicGroupResponse) Either.<BusinessException, Tsid>right(id)
                ...
                .flatMap(this::deleteEconomicGroup)
                .map(EconomicGroupResponse::from)
                .fold(this::throwBusinessException, group -> group);
    }

    private Either<? extends BusinessException, EconomicGroup> deleteEconomicGroup(EconomicGroup economicGroup) {
        economicGroup.deleted();
        economicGroupRepository.delete(economicGroup);
        return Either.right(economicGroup);
    }

    private Either<? extends BusinessException, EconomicGroup> findEconomicGroup(Tsid id) {
        return economicGroupRepository.findById(id.toLong())
